// MUI Imports
import Card from '@mui/material/Card'
import Button from '@mui/material/Button'
import CardContent from '@mui/material/CardContent'
import Typography from '@mui/material/Typography'

// Component Imports

import OpenDialogOnElementClick from './open-dialog-on-element-click'
import EditDivisionInfo from './edit-division-info'

const DialogEditUserInfo = () => {
  // Vars
  const buttonProps = {
    variant: 'contained',
    children: 'Show'
  }

  return (
    <>
      <Card>
        <CardContent className='flex flex-col items-center text-center gap-4'>
          <i className='tabler-user text-[34px] text-textPrimary' />
          <Typography variant='h5'>Edit User Info</Typography>
          <Typography color='text.primary'>
            Use this modal to modify the existing user&#39;s current information.
          </Typography>
          <OpenDialogOnElementClick element={Button} elementProps={buttonProps} dialog={EditDivisionInfo} />
        </CardContent>
      </Card>
    </>
  )
}

export default DialogEditUserInfo
