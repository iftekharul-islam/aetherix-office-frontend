...
import UserTableFilters from './UserTableFilter'
...
const UserListTable = ({ tableData, departmentsData, usersData }) => {
  // States
  ...
  const columns = useMemo(
    () => [
      ...
      columnHelper.accessor('name', {
        ...
        cell: ({ row }) => (
          <div className='flex items-center gap-4'>
            {getAvatar({ fullName: row.original.name })}
      ...
      columnHelper.accessor('employee_id', {
        header: 'Employee ID',
        cell: ({ row }) => <Typography>{row.original.employee_id}</Typography>
      }),
      columnHelper.accessor('machine_id', {
        header: 'Machine ID',
        cell: ({ row }) => <Typography>{row.original.machine_id ?? '-'}</Typography>
      }),
      ...
      columnHelper.accessor('division', {
        header: 'Division',
        cell: ({ row }) => <Typography>{row.original.department?.division?.name ?? '-'}</Typography>
      }),
      columnHelper.accessor('supervisor', {
        header: 'Supervisor',
        cell: ({ row }) => <Typography>{row.original.supervisor?.name ?? '-'}</Typography>
      }),
      columnHelper.accessor('role', {
        header: 'Role',
        cell: ({ row }) => <Typography className='capitalize'>{row.original.role ?? '-'}</Typography>
      }),
